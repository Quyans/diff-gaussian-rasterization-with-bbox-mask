	...
	const bool debug)
{
  ...
  torch::Tensor masks = torch::full({P}, 0, means3D.options().dtype(torch::kInt32));
  ...
  return std::make_tuple(rendered, out_color, out_depth, out_alpha, radii, masks, geomBuffer, binningBuffer, imgBuffer);
}
